...
def generate_synthetic_data(n_samples: int = 2000) -> list[dict]:
    ...
    def _base() -> dict:
        ...
        return {
            ...
            "mean_intensity":         float(rng.uniform(70, 172)),
            "intensity_std":          float(rng.uniform(36, 62)),
            "underexposed_ratio":     float(rng.uniform(0.0, 0.10)),
            "overexposed_ratio":      float(rng.uniform(0.0, 0.12)),
            "optical_flow_magnitude": float(rng.uniform(0.0, 7.0)),
            ...
            "small_object_ratio":     float(rng.uniform(0.0, 0.40)),
            "edge_density":           float(rng.uniform(0.0, 0.12)),
    ...
    for _ in range(n_per):
        f = _base()
        f["intensity_std"]      = float(rng.uniform(5, 30))
